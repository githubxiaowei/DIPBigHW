...
function prepare(feat_func,feat_i)
    ...
    if ~exist(g_bird_data.features.paths{feat_i},'file')
        ...
        for i = 1:g_bird_data.img_num
            features{i} = feat_func(imread(g_bird_data.img_paths{i})); 
            waitbar(i/g_bird_data.img_num);
        end
        close(h);
        fprintf(['--- saving ',g_bird_data.features.classes{feat_i},' features ...\n']);
        save(g_bird_data.features.paths{feat_i},'features');
    end
    fprintf(['--- complete ',g_bird_data.features.classes{feat_i},' feature preparing!\n']);
end
